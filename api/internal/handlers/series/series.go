...
// Item is one row in the cross-library list.
type Item struct {
	ID           string `json:"id"`
...
func (h *Handler) loadSeriesHeaders(r *http.Request, libraryFilter string) ([]Item, error) {
...
func sortSeries(items []Item, key string) {
	switch key {
	case "progress":
...
func progressRatio(s Item) float64 {
	if s.EpisodeCount == 0 {
		return 0
	}
