...
| Concern | Decision |
|---|---|
| Framework | React 18, Vite, TypeScript, TanStack Router (file-based routes), Tailwind CSS, TanStack Query v5, TanStack Virtual. |
| Placement | `web/src/routes/library/` (route + page), `web/src/features/library/` (hooks + components). |
| Data fetching | `useLibraryVideos(filters, cursor)` — wraps `GET /api/videos?library_id={id}` (Epic 7 Story 7.4, architecture §9.2) via TanStack Query infinite query. |
| URL state | `?lang=ar&type=lecture&duration=10-30&speaker=…&tag=…&library=…&sort=recent&view=grid` parsed by `useLibraryFilters()` (custom hook over `Route.useSearch()` from TanStack Router). |
...
| `web/src/features/library/useLibraryFilters.ts` | URL ↔ filter-state hook; emits `LibraryFilters` and `setFilter(name, value)`. Uses TanStack Router's `Route.useSearch()` + `Route.useNavigate()`. |
| `web/src/features/library/useLibraryVideos.ts` | Infinite query hook over `GET /api/videos?library_id={id}`. |
...
```tsx
// web/src/routes/library/$libraryId.tsx (TanStack Router file-based)
import { createFileRoute } from '@tanstack/react-router';

export const Route = createFileRoute('/library/$libraryId')({
  component: () => (
    <Suspense fallback={<LibrarySkeleton/>}><LibraryPage/></Suspense>
  ),
  // optional: validateSearch, loader, pendingComponent, etc.
});
```

`useLibraryVideos` is a TanStack `useInfiniteQuery` keyed by serialized
filters, including `library_id`. `getNextPageParam` returns
`lastPage.next_cursor` (Epic 7 Story 7.2 contract). On filter change,
the cursor is reset by changing the query key — we never hand-clear the
cache.

```ts
const queryKey = ['videos', { library_id: filters.libraryId, ...filtersKey(filters) }];
useInfiniteQuery({
  queryKey,
  queryFn: ({ pageParam }) =>
    fetchVideos({ library_id: filters.libraryId, ...filters, cursor: pageParam, limit: 60 }),
...
`fetchVideos` issues `GET /api/videos?library_id={id}&...` (architecture §9.2). There is no `GET /api/libraries/{id}/videos` endpoint; library scoping is just another filter on the videos collection.

### 4.2 Filter URL plumbing

`useLibraryFilters` reads `Route.useSearch()` once; mutating any filter
calls `Route.useNavigate()({ search: (prev) => next })` so links are
shareable. Multi-value chips are encoded as arrays in TanStack Router's
typed search state (validated via `validateSearch` on the route);
serialization of repeated keys (`?lang=ar&lang=en`) is handled by the
...
| Test | Asserts |
|---|---|
| `parses url filters and round-trips through router search` | `?lang=ar&lang=en&type=lecture` → `{ lang:['ar','en'], type:['lecture'] }` and back via TanStack Router. |
...
| `chip remove emits correct navigate({ search })` | Removing `lang=ar` updates URL to `?lang=en&type=lecture`. |
