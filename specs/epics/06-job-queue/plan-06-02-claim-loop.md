...
_CLAIM_SQL_PG = """
-- Both 'pending' and 'paused' rows must respect pause_requested = false
-- (plan-06-04 §7 cross-reference: a pause requested before claim is
-- honored even on a never-claimed row). Without this, a fresh pending
-- row whose user clicked Pause before any worker picked it up would
-- still get claimed.
...
| `test_claim_respects_paused_pause_requested_combinations` | Truth-table: state ∈ {pending, paused, claimed, running, done, failed, cancelled} × pause_requested ∈ {true, false} → only (pending, false) and (paused, false) are claimable. |
| `test_claim_skips_pending_with_pause_requested` | Insert `state='pending', pause_requested=true` → claim returns `None`; the row stays pending until `pause_requested` is cleared. |
