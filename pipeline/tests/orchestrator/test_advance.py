...
async def test_advance_canonical_edge(frm: State, trig: Trigger, out: Outcome, to: State) -> None:
...
async def test_subtitle_gen_does_not_advance_to_indexed_alone() -> None:
    ...
    got = await advance_after_stage(db, VIDEO_ID, Trigger.SUBTITLE_GEN, Outcome.PARTIAL, log=log)
    ...
    got = await advance_after_stage(db, VIDEO_ID, Trigger.INDEX, Outcome.OK, log=log)
...
async def test_corrupted_blocks_further_processing() -> None:
    ...
    got = await advance_after_stage(db, VIDEO_ID, Trigger.THUMBNAIL, Outcome.OK, log=log)
...
async def test_advance_rejects_invalid_outcome() -> None:
    ...
    with pytest.raises(IllegalStateTransition) as exc_info:
        await advance_after_stage(db, VIDEO_ID, Trigger.PROBE, "weird", log=log)
...
async def test_advance_rejects_invalid_trigger_for_state() -> None:
    ...
    with pytest.raises(IllegalStateTransition):
        await advance_after_stage(db, VIDEO_ID, Trigger.TRANSCRIBE, Outcome.OK, log=log)
    assert db.current_state is State.DISCOVERED
    assert db.executes == []
...
async def test_advance_rejects_extract_on_no_audio() -> None:
    ...
    with pytest.raises(IllegalStateTransition):
        await advance_after_stage(db, VIDEO_ID, Trigger.EXTRACT, Outcome.OK, log=log)
...
async def test_missing_to_discovered_round_trip() -> None:
    ...
    got = await advance_after_stage(db, VIDEO_ID, Trigger.SCAN, Outcome.REDISCOVERED, log=log)
...
async def test_filesystem_deleted_drives_to_missing() -> None:
    ...
    got = await advance_after_stage(db, VIDEO_ID, Trigger.FILESYSTEM, Outcome.DELETED, log=log)
...
async def test_library_replaced_drives_to_superseded() -> None:
    ...
    got = await advance_after_stage(db, VIDEO_ID, Trigger.LIBRARY, Outcome.REPLACED, log=log)
...
async def test_integrity_fail_drives_to_corrupted() -> None:
    ...
    got = await advance_after_stage(db, VIDEO_ID, Trigger.INTEGRITY, Outcome.FAIL, log=log)
    assert got is State.CORRUPTED
...
async def test_exhausted_drives_to_failed() -> None:
    ...
    got = await advance_after_stage(db, VIDEO_ID, Trigger.TRANSCRIBE, Outcome.EXHAUSTED, log=log)
...
async def test_sqlite_publishes_state_changed_on_bus() -> None:
    ...
    await advance_after_stage(db, VIDEO_ID, Trigger.PROBE, Outcome.OK, log=log)
...
def test_serialize_notify_payload_shape() -> None:
    out = json.loads(serialize_notify_payload(VIDEO_ID, LIBRARY_ID, State.DISCOVERED, State.PROBED))
    assert out == {
...
def test_serialize_notify_payload_handles_null_library_id() -> None:
    out = json.loads(serialize_notify_payload(VIDEO_ID, None, State.PROBED, State.MISSING))
