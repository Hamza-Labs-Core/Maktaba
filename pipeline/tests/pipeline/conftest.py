...
class StageDB(FakeAudioDB):
    """:class:`FakeAudioDB` plus a ``videos.path`` projection.

    The PROBE adapter resolves the on-disk source file with
    ``SELECT path FROM videos WHERE id = $1`` before shelling out to
    ffprobe. ``FakeAudioDB`` only modelled ``state`` for that table, so
    we extend its dispatch with the one extra column and keep a
    side-table of paths the test seeds via :meth:`add_video`.
    """
