...
def parse(filename: str, *, dirnames: Sequence[str] = ()) -> ParsedTitle:
    ...
    if resolution is None and _RESOLUTION_4K_RE.search(stem):
        resolution = "2160p"
    source = _first(
        stem, _SOURCE_RE, lambda m: _SOURCE_CANON.get(m.group(1).lower(), m.group(1).lower())
    )
    video_codec = _first(
        stem,
        _VCODEC_RE,
        lambda m: _VCODEC_CANON.get(m.group(1).lower().replace(".", ""), m.group(1).lower()),
    )
    audio_codec = _first(stem, _ACODEC_RE, lambda m: m.group(1).lower())
    edition = _first(
        stem,
        _EDITION_RE,
        lambda m: _EDITION_CANON.get(_norm_edition(m.group(1)), _norm_edition(m.group(1))),
    )
