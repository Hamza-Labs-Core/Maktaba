...
| `0063` | `0063_playback_state_continue_partial_idx.sql` | gap-closure Wave 3 (Epic 14 / Story 14.5) | 0038 | Partial covering index `playback_state_continue_partial_idx` `(user_id, updated_at DESC) INCLUDE (video_id, position_sec) WHERE completed = false` — Story 14.5 explicitly owns this partial index so the cross-device Continue Watching rail is index-only and never sequential-scans. SQLite sibling folds the INCLUDE columns into the key tuple (no INCLUDE clause). |
| `0064` | `0064_recommendation_dismissals.sql` | gap-closure Wave 3 (Epic 14 / Story 14.7) | 0038, 0041 | `recommendation_dismissals` (persisted "Not interested" hides — `(user_id, reason_kind, video_id?)`, ON DELETE CASCADE on user/video, unique index with all-zero-UUID sentinel for the NULL video_id row scope + per-user index). Filtered by the recommendations composer so hides persist cross-device. SQLite sibling uses an empty-string sentinel. |
