...
export function SeriesDetail() {
  ...
  useEffect(() => {
    ...
    api.get<SeriesHeader>(`/api/series/${enc}`).then(setHeader).catch(() => setHeader(null));
    api
      .get<EpisodesResponse>(`/api/series/${enc}/episodes`)
      .then((r) => setData({ seasons: r.seasons ?? [], numbering: r.numbering, next_episode: r.next_episode }))
                ...
                <li key={ep.video_id}>
                  <Link to={`/videos/${ep.video_id}`} className="mkt-episode">
                    <div className="mkt-episode__thumb">
                      ...
                      {ep.watched && <span className="mkt-episode__watched" aria-hidden="true">✓</span>}
                    </div>
                    ...
                    {ep.progress_pct > 0 && !ep.watched && (
                      <ProgressBar
                        ...
                        label={t("series.episode.progress", { pct: String(Math.round(ep.progress_pct)) })}
                      />
                    )}
                  </Link>
                </li>
              ...
              {(gapsBySeason.get(s.season) ?? []).map((epNo) => (
                <li key={`gap-${s.season}-${epNo}`}>
                  <div className="mkt-episode mkt-episode--missing" aria-disabled="true">
                    <div className="mkt-episode__thumb mkt-episode__thumb--missing" aria-hidden="true">
                      ?
                    </div>
