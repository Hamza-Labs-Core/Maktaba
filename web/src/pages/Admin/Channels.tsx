  ...
  return (
    <section className="mkt-page mkt-channels">
      <header className="mkt-page__header">
        ...
        <Button onClick={() => setEditing(blankChannel(nextNumber))}>{t("channels.new")}</Button>
      </header>
      ...
      {fillerFor && <FillerManager channel={fillerFor} onClose={() => setFillerFor(null)} />}
    </section>
  );
}
...
function ChannelForm({
  ...
  const numberCollision = existing.some((c) => c.number === form.number && c.id !== draft.id);
  ...
  return (
    <Modal open onClose={onClose} title={isEdit ? t("channels.edit") : t("channels.new")}>
      <div className="mkt-form">
        ...
        <RuleBuilder
          mode={form.mode ?? "shuffle"}
          config={form.mode_config ?? {}}
          setConfig={setConfig}
        />
