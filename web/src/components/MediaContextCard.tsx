...
function ratingText(rating: Facts["rating"]): string | null {
  ...
  const parts = Object.entries(rating).map(
    ([k, v]) => `${k.toUpperCase()} ${Number(v).toFixed(1)}`
  );
...
export function MediaContextCard({ videoId }: { videoId: string }) {
    ...
    api
      ...
      .then((r) => {
        if (alive)
          setData({
            ...r,
            related_in_library: r.related_in_library ?? [],
            more_like_this: r.more_like_this ?? [],
          });
      })
      .catch(() => {
  ...
  const hasFacts = Boolean(
    facts &&
      (rating || facts.summary || facts.cast?.length || facts.genres?.length || facts.runtime_min)
  );
  ...
  return (
    <Card className="mkt-context-card" elevation={1}>
      ...
      {data.related_in_library.length > 0 && (
        <RelatedRail
          title={t("context.related")}
          items={data.related_in_library}
