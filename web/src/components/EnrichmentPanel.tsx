...
export function EnrichmentPanel({ videoId }: { videoId: string }) {
  ...
  async function accept(c: Candidate) {
    ...
    try {
      ...
      toast.show({ message: e instanceof ApiError ? e.problem.title : t("common.error"), tone: "error" });
    } finally {
      setBusy(false);
    }
  }
    ...
    <Card className="mkt-enrich-panel" elevation={1}>
      <div className="mkt-enrich-panel__search">
        <input
          ...
          dir="auto"
        />
        <Button variant="secondary" onClick={manualSearch} loading={searching} disabled={!query.trim()}>
      ...
      {candidates.length === 0 ? (
        ...
        <>
          <p className="mkt-enrich-panel__headline" dir="auto">
            ...
            <Badge tone="accent">{t("enrich.matchPct", { pct: String(Math.round(top.confidence * 100)) })}</Badge>
          </p>
